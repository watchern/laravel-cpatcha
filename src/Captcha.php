...
class Captcha
{
    ...
    )
    {
        ...
        $this->hasher = $hasher;
        $this->str = $str;
        $this->mode = config('captcha.mode', 'session');
        $this->lifetime = config('captcha.lifetime', 60);
    ...
    protected function generate(): array
    {
        ...
        if ($this->mode === 'session') {
            ...
            $hash = $this->encode($key, '', $this->lifetime);
        }
     ...
     * @param string $str 原文
     * @param string $key 密钥
     * @param int $expiry 密文有效期，默认60秒
     * @return string 处理后的 原文或者 经过 base64_encode 处理后的密文
     */
    public function encode($str, $key, $expiry = 60)
    {
        return $this->_auth_code($str, 'ENCODE', $key, $expiry);
    }
    ...
    {
        return $this->_auth_code($str, 'DECODE', $key);
    }
     ...
     */
    public function _auth_code($string, $operation = 'DECODE', $key = '', $expiry = 3600)
    {
        /***
         * 随机密钥长度 取值 0-32;
        ...
        $string = $operation == 'DECODE' ? base64_decode(substr($string, $ckey_length)) : sprintf('%010d', $expiry ? $expiry + time() : 0) . substr(md5($string . $key_b), 0, 16) . $string;
